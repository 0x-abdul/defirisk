...
INCOMPLETE_V17_RECOVERY_SCHEMA = (
    "lean-protocol-refresh/incomplete-v17-recovery/v1"
)
...
@dataclass(frozen=True)
class IncompleteV17Recovery:
    """Bound one-or-more-missing-current-v1.7 recovery before refresh."""

    schema_version: str
    source_rubric_version: str
    target_rubric_version: str
    selection_policy: str
    missing_target_scope_keys: tuple[str, ...]
    full_target_projection: tuple[FactorChange, ...]
    full_target_projection_semantic_sha256: str
    protocol_change_semantic_sha256: str
...
class ProtocolRefresh:
    ...
    incomplete_v17_recovery: IncompleteV17Recovery | None = None
...
def _incomplete_v17_recovery(
    ...
    *,
    ...
    changes: tuple[FactorChange, ...],
    protocol_change_semantic_sha256: str,
) -> IncompleteV17Recovery:
    """Validate the narrow current-v1.7 coverage-repair envelope.

    This is deliberately distinct from a v1.5-to-v1.7 mixed migration: it
    binds a complete public target projection while requiring the adapter to
    retain every existing v1.7 row and insert only the named absent scopes.
    """
    if not isinstance(value, dict):
        raise ContractError(f"{label} must be an object")
    _exact_fields(
    ...
    if value["schema_version"] != INCOMPLETE_V17_RECOVERY_SCHEMA:
        raise ContractError(f"{label}.schema_version is invalid")
    if (
        value["source_rubric_version"] != RUBRIC_VERSION
        or value["target_rubric_version"] != RUBRIC_VERSION
    ):
        raise ContractError(
            f"{label} must declare the supported {RUBRIC_VERSION} coverage-repair route"
        )
    if value["selection_policy"] != "retain_current_then_insert_missing":
        raise ContractError(f"{label}.selection_policy is invalid")
    missing = value["missing_target_scope_keys"]
    if (
        ...
        raise ContractError(
            f"{label}.missing_target_scope_keys must contain one canonical scoped key"
        )
    expected_projection_hash = value["full_target_projection_semantic_sha256"]
    projection = value["full_target_projection"]
    if (
        not isinstance(projection, list)
        or not isinstance(expected_projection_hash, str)
        or not re.fullmatch(r"[0-9a-f]{64}", expected_projection_hash)
        or _semantic_sha256(projection) != expected_projection_hash
    ):
        raise ContractError(
    ...
    projection_keys = {"|".join(key) for key in sort_keys}
    if set(missing) - projection_keys:
        raise ContractError(f"{label}.missing_target_scope_keys is outside the target projection")
    projected = {
        (row.scope_level, row.target, row.factor_id): row.new_value
        for row in parsed_rows
    }
    ...
    return IncompleteV17Recovery(
        schema_version=value["schema_version"],
        source_rubric_version=value["source_rubric_version"],
        target_rubric_version=value["target_rubric_version"],
        selection_policy=value["selection_policy"],
        missing_target_scope_keys=tuple(missing),
        full_target_projection=tuple(parsed_rows),
        full_target_projection_semantic_sha256=expected_projection_hash,
        protocol_change_semantic_sha256=expected_protocol_hash,
    )
...
def _protocol(value: Any, label: str, rubric_version: str) -> ProtocolRefresh:
    ...
    incomplete_v17_recovery_raw = value.pop("incomplete_v17_recovery", None)
    if mixed_recovery_raw is not None and incomplete_v17_recovery_raw is not None:
        raise ContractError(f"{label} cannot contain both recovery envelopes")
    ...
    incomplete_v17_recovery = (
        _incomplete_v17_recovery(
            incomplete_v17_recovery_raw,
            f"{label}.incomplete_v17_recovery",
            family_slug=family_slug,
            surface_slugs=surface_slugs,
            deployment_targets=deployment_targets,
            resulting_grade=resulting_grade,
        ...
        if incomplete_v17_recovery_raw is not None
        else None
    )
    return ProtocolRefresh(
        ...
        incomplete_v17_recovery=incomplete_v17_recovery,
    )
